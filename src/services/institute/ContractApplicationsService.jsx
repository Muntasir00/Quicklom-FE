...
export const getContractApplicationCandidatesService = async (applicationId) => {
    try {
        const response = await axios.get(`/v.1/${API_RESOURCE}/contract-applications/${applicationId}/candidates`);
        ...
        Swal.fire("Error", "Failed to fetch candidates information.", "error");
        return null;
    }
};
...
/**
 * Get unmasked candidates data for applications owned by the current user (agency/headhunter)
 * This returns full candidate details without sanitization since the user submitted them
 */
export const getMyCandidatesService = async (applicationId) => {
    try {
        const response = await axios.get(`/v.1/${API_RESOURCE}/contract-applications/${applicationId}/my-candidates`);
