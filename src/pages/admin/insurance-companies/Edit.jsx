...
import { updateInsuranceCompanyService, getInsuranceCompanyByIdService } from "../../../services/admin/InsuranceCompanyService";
...
const Edit = () => {
    ...
    const onSubmit = async (data) => {
        try {
            const success = await updateInsuranceCompanyService(Number(id), data);
            if (success) navigate(`/${userRole}/insurance-companies`);
        } catch (err) {
            if (err.response?.data?.errors) {
                Object.keys(err.response.data.errors).forEach(field => {
                    setError(field, { type: "server", message: err.response.data.errors[field] });
                });
    ...
    const fetchData = async () => {
        const data = await getInsuranceCompanyByIdService(Number(id));
        if (!data) return;
        reset({
            province_territory: data?.province_territory ?? "",
    ...
    return (
        <Box sx={{ minHeight: '100vh', backgroundColor: '#f8fafc' }}>
            <PageHeader
                title={`Edit ${menu}`}
                subtitle="Update insurance company details"
